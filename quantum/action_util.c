...
#    define ACTION_UTIL_QS_ONESHOT_TIMEOUT_TRACKING
#    define ACTION_UTIL_QS_ONESHOT_TIMEOUT QS_oneshot_timeout
#elif defined(ONESHOT_TIMEOUT) && (ONESHOT_TIMEOUT > 0)
#    define ACTION_UTIL_QS_ONESHOT_TIMEOUT_TRACKING
#    define ACTION_UTIL_QS_ONESHOT_TIMEOUT ONESHOT_TIMEOUT
#endif
#include <string.h>
...
#    ifdef ACTION_UTIL_QS_ONESHOT_TIMEOUT_TRACKING
static uint16_t oneshot_time = 0;
bool            has_oneshot_mods_timed_out(void) {
    return ACTION_UTIL_QS_ONESHOT_TIMEOUT > 0 && TIMER_DIFF_16(timer_read(), oneshot_time) >= ACTION_UTIL_QS_ONESHOT_TIMEOUT;
}
#    else
bool has_oneshot_mods_timed_out(void) {
    return false;
...
inline bool     has_oneshot_layer_timed_out(void) {
    return ACTION_UTIL_QS_ONESHOT_TIMEOUT > 0 && TIMER_DIFF_16(timer_read(), oneshot_layer_time) >= ACTION_UTIL_QS_ONESHOT_TIMEOUT && !(get_oneshot_layer_state() & ONESHOT_TOGGLED);
}
#        ifdef SWAP_HANDS_ENABLE
static uint16_t oneshot_swaphands_time = 0;
inline bool     has_oneshot_swaphands_timed_out(void) {
    return ACTION_UTIL_QS_ONESHOT_TIMEOUT > 0 && TIMER_DIFF_16(timer_read(), oneshot_swaphands_time) >= ACTION_UTIL_QS_ONESHOT_TIMEOUT && (swap_hands_oneshot == SHO_ACTIVE);
}
#        endif
#    endif
